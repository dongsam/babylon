...
// TestHeaderCache_NoEviction tests that cache doesn't evict headers (no size limit)
func TestHeaderCache_NoEviction(t *testing.T) {
	cache := types.NewHeaderCache()
	...
	require.Equal(t, int64(1000), stats.MissCount)

	// Test that first entry can still be accessed (cache hit)
	_, err := cache.GetOrFetch(1, fetcher)
	require.NoError(t, err)
	require.Equal(t, 1000, fetchCount) // Should not increment

	stats = cache.Stats()
	require.Equal(t, int64(1), stats.HitCount)
}

// TestHeaderCache_NoExpiration tests that cache doesn't expire headers (no age limit)
func TestHeaderCache_NoExpiration(t *testing.T) {
	cache := types.NewHeaderCache()
	...
	// Wait a bit and verify cache still works (no expiration)
	time.Sleep(100 * time.Millisecond)
	_, err = cache.GetOrFetch(100, fetcher)
	require.NoError(t, err)
	require.Equal(t, 1, fetchCount) // Still should not increment
}
...
func TestGetMainChainFrom_TipChanges(t *testing.T) {
	...
	// Get initial result and cache stats using actual heights
	baseHeight := headers[0].Height
	tipHeight := headers[numHeaders-1].Height
