...
// This can be called from IBC channel creation or consumer registration events
...
func (k Keeper) ShouldBroadcastBTCHeaders(ctx context.Context) bool {
	...
	k.Logger(sdkCtx).Debug("No BTC header broadcasting triggers found, skipping broadcast")
	return false
}

// GetBroadcastTriggerReason returns the reason for broadcasting (for logging/metrics, to be removed)
